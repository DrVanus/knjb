...
struct PortfolioView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showAddSheet) {
            // When editing, the viewModel.editingTransaction is used
...
extension PortfolioView {
    ...
    private var transactionsTab: some View {
        VStack {
            ...
            // Use a List so we can easily add swipe actions.
            List {
                ForEach(viewModel.transactions) { tx in
                    TransactionsRow(symbol: tx.coinSymbol,
                                    quantity: tx.quantity,
                                    price: tx.pricePerUnit,
                                    date: tx.date,
                                    isBuy: tx.isBuy)
                    .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                        // Allow edit/delete only if the transaction was manually entered.
                        if tx.isManual {
        ...
        ZStack {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    // Display total portfolio value (update these values with your actual calculations)
                    Text("$65,000.00")
                        .font(.system(size: 32, weight: .bold))
                        .foregroundColor(.white)
                    Text("Total Value: $65,000.00")
                ...
                if #available(iOS 16.0, *) {
                    VStack(spacing: 6) {
                        // Donut pie chart showing coin distribution from your separated view.
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            LazyVStack(spacing: 8) {
                ForEach(displayedHoldings) { holding in
                    // This view is defined in PortfolioCoinRow.swift
