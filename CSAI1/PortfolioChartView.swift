...
class PortfolioChartViewModel: ObservableObject {
    ...
    // For now, loadData creates a constant (or simulated) line based on the portfolio's current total.
    func loadData(for range: TimeRange, portfolioTotal: Double) {
        let now = Date()
        self.totalValue = portfolioTotal
        
        // Choose the number of data points based on the selected range.
        let count: Int = {
            switch range {
            case .day: return 24
            case .week: return 7
        ...
        // (Later you can update this to simulate real historical variation from transaction data.)
...
struct PortfolioChartView: View {
    ...
    var body: some View {
        ...
        // New: Update chart data when the portfolio total changes.
        .onChange(of: portfolioVM.totalValue) { newValue in
            vm.loadData(for: vm.selectedRange, portfolioTotal: newValue)
        }
    }
